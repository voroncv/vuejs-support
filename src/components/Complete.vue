<template>
  <div class="complete">
    <div class="col-md-4">
        <ul class="list-group" >
          <a class="list-group-item" @click="onselect(item, false, false)" v-for="item in completeTickets">
            <p><b>{{ item.title }} </b></p>
            <p>ID: {{ item.id }}</p>
            <p>{{ item.author }}</p>
          </a>
      ...
      <div class="col-md-8">
      <h1 v-if="completeTickets.length <= 0" class="text-center">Ничего не найдено!</h1>
        <h1 v-else="selectItem.length <= 0" v-show="selectItem.length <= 0" class="text-center">Выберите диалог слева чтобы начать</h1>
        <div v-if="selectItem">
          <div v-for="item in selectItem" v-if="completeTickets.length > 0">
            <ol class="breadcrumb">
